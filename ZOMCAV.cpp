...
//GFG Ref:
vector<int> initializeDiffArray(vector<int>& A, int n) { 
    vector<int> D(n + 1); 
    D[0] = A[0], D[n] = 0; 
    for (int i = 1; i < n; i++) 
        D[i] = A[i] - A[i - 1]; 
...
int main() 
{
    ...
    while(t--) {
    	int n;
    	cin >> n;
    	vector<int> C(n), H(n), R(n, 0);
    	vector<int> D = initializeDiffArray(R, n);
    	for(int& i: C) cin >> i;
    	for(int& i: H) cin >> i;
    	int start = 0;
    	while(true) {
    		if(start == n) break;
    		int l = (start + 1) - C[start];
    		int r = (start + 1) + C[start];
    		if(l < 1) l = 1;
    		if(r > n) r = n;
    		update(D, l - 1, r - 1, 1);
    		start++;
