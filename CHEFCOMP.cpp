...
vector<int> reachN;
void DFS(int start, int n, vector<vector<int> > &adj, vector<bool> &visited) {
    reachN.PB(start);
    visited[start] = true;
    for(int i = 0; i < n; i++)
...
void removeV(int n, int v, vector<vector<int> > &adj) {
    for(int i = 0; i < n; i++) {
        adj[i][v] = 0;
        adj[v][i] = 0;
    }
...
int main()  { 
    ...
    while(t--) {
    	...
    	for(int i: P) {
    		...
    		DFS(i - 1, n, adj, visited);
    		for(int j: reachN) {
    			B[j] -= min(B[j], A[i - 1]);
    		}
    		for(int j = 0; j < n; j++) {
    			if(B[j] == 0 && days[j] == -1) days[j] = day;
    		}
    		removeV(n, i - 1, adj);
    		day++;
    		reachN.clear();
